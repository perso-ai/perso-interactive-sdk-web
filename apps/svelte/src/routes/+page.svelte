	...
	async function createSession() {
		try {
			...
			if (response.ok) {
				...
				sessionError = `Session creation failed: ${response.status}`;
			}
		} catch (error) {
			sessionError = `Session creation failed: ${(error as Error).message}`;
		}
	}
...
<svelte:head>
	<title>SDK Playground — Svelte</title>
</svelte:head>
...
<div class="container">
	...
	{#if sessionError}
		<p class="status error">{sessionError}</p>
	{:else if persoInteractiveConfig != null}
		<LiveChat {persoInteractiveConfig} />
	{:else}
